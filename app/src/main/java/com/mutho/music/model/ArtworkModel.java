    ...
    public static String getTypeString(@ArtworkProvider.Type int type) {
        switch (type) {
            case ArtworkProvider.Type.MEDIA_STORE:
                return MuthoMusicApplication.getInstance().getString(R.string.artwork_type_media_store);
            case ArtworkProvider.Type.TAG:
                return MuthoMusicApplication.getInstance().getString(R.string.artwork_type_tag);
            case ArtworkProvider.Type.FOLDER:
                return "Folder";
            case ArtworkProvider.Type.REMOTE:
                return MuthoMusicApplication.getInstance().getString(R.string.artwork_type_internet);
        }
        return null;
    }
