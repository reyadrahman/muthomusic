...
public class SafManager {
    ...
    public DocumentFile getDocumentFile(final File file) {
        ...
        DocumentFile document = DocumentFile.fromTreeUri(MuthoMusicApplication.getInstance(), Uri.parse(treeUri));
    ...
    public String getDocumentTree() {
        ...
        List<UriPermission> perms = MuthoMusicApplication.getInstance().getContentResolver().getPersistedUriPermissions();
    ...
    private List<String> getExtSdCardPaths() {
        ...
        try {
            File[] externalFilesDirs = MuthoMusicApplication.getInstance().getExternalFilesDirs("external");
            if (externalFilesDirs != null && externalFilesDirs.length > 0) {
                for (File file : externalFilesDirs) {
                    if (file != null && !file.equals(MuthoMusicApplication.getInstance().getExternalFilesDir("external"))) {
            ...
            switch (requestCode) {
                case DOCUMENT_TREE_REQUEST_CODE:
                    if (resultCode == Activity.RESULT_OK) {
                        ...
                        if (treeUri != null) {
                            MuthoMusicApplication.getInstance().getContentResolver().takePersistableUriPermission(treeUri, Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
